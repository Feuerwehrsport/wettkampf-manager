...
class TextElement
  constructor: (@formElement) ->
    ...
    @addButton = $('<div/>').addClass('btn btn-default btn-xs btn-block').
      appendTo($('#position-buttons')).text(@formElement.description()).click(@show)
  ...
  setAlign: (buttonLine) =>
    ...
    @textAlignButton = $('<div/>').addClass("btn btn-default btn-xs glyphicon glyphicon-align-#{@currentAlign()}").
      appendTo(buttonLine)
