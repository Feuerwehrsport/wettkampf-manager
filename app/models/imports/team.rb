...
Imports::Team = Struct.new(:configuration, :import_band, :data) do
  def import
    ...
    data[:assessments].each do |assessment_foreign_key|
      assessment = import_band.assessments.find { |a| a.foreign_key == assessment_foreign_key }.try(:assessment)
